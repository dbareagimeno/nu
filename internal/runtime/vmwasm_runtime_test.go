...
// M13d.1: enu.version.api por el Runtime → "4" (el APILevel que buildWasmState
// inyecta con SetAPIVersion). Es el smoke test que el binario `enu -e` reproduce.
func TestRuntimeWasmVersionAPI(t *testing.T) {
	rt := newWasmRuntime(t)
	if got := evalStringOne(t, rt, `return enu.version.api`); got != "4" {
		t.Fatalf("enu.version.api = %q, esperado 4", got)
	}
}
