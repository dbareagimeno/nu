...
description: Run Lua with nu -e, understand the main state versus tasks, and handle structured errors.
---

`nu` is, above all, a Lua runtime. The fastest way to try it is `nu -e`,
...
In `nu`, `print` is an alias for `nu.log.info`: it writes to the log file,
**never to the screen** (the UI belongs to extensions, not the core). For
something to show up on stdout with `nu -e`, `return` it.
:::
...
The `nu -e` chunk runs in the **main state** (single-threaded, with an event
loop). The main state **is not a task**, so it **cannot call suspending
functions** (the ones marked with ⏸: almost all IO —`nu.fs.read`,
`nu.http.request`, `nu.proc.run`…). If you try:
...
```
error: EINVAL: nu.fs.read can only be called inside a task
```

To do IO, launch a **task** with
[`nu.task.spawn`](/nu/en/api/task/). Inside a task, ⏸ functions are
...
nu.task.spawn(function()
  local texto = nu.fs.read("README.md")   -- ⏸ this is fine here
...
`nu -e` waits for **all** tasks spawned by the chunk to finish before
...
the `message`. See
[Conventions](/nu/en/api/convenciones/#errors) for the complete list.

## Exit code

`nu -e` exits with **0** if the chunk didn't throw, and with **1** if it did
(an uncaught error). Useful in scripts and CI:

```sh
nu -e 'assert(nu.version.api >= 2)' && echo "API sufficient"
```
...
agent](/nu/en/docs/primer-agente/). If you'd rather get the full mental
model, go to [Key concepts](/nu/en/docs/conceptos/).
