---
title: The CLI
description: The nu binary's flags, headless modes and exit codes.
---

This page documents the **command-line surface** of the `nu` binary.
It is not sacred `nu.*` API (that's the Lua surface): it's the interface of the
...
```
nu                       Canonical boot. With a TTY and no active plugins,
                         paints the bare runtime screen (G21).
nu --default-config      Activates the official product set without a TTY: writes
                         plugins.enabled to nu.toml and exits (with -p/-e, it activates
                         it only for that process, without touching disk).
nu -e '<lua>'            Evaluates a headless Lua chunk and prints its returns.
nu -p '<prompt>'         Runs a headless agent turn and prints the assistant's
...
screen: it prints usage. The no-TTY equivalent is `nu --default-config`.

### `nu --default-config`

The **no-TTY onramp** for having nu *batteries-included* in CI, Docker or scripts,
...
- **Alone** (`nu --default-config`): **writes** `plugins.enabled` to
  `config.dir()/nu.toml` and exits. Preserves the rest of the file (other keys,
  `[watchdog]`, …), is **atomic** (doesn't leave a half-written `nu.toml`) and
  **idempotent** (repeating it changes nothing). If the existing `nu.toml` is
...
nu --default-config
nu -p 'resume este repo'        # agent already active
...
surface, not sacred `nu.*` API (it adds nothing to the API nor bumps `nu.version.api`).
...
`nu.task.spawn` but not use ⏸ functions directly. See [Your first
...
active. See [Your first agent](/nu/en/docs/primer-agente/).
...
|---|---|
| **0** | Success. |
| **1** | Execution error: the `-e` chunk, the agent turn or the provider threw, or boot failed (invalid plugin graph, broken `nu.toml`). |
...
`nu --default-config` (persistent mode) exits with **0** after writing, or with **1**
if it couldn't write `nu.toml` (e.g. the existing file is malformed and isn't
...
:::note[Windows]
`nu` is used on Windows via **WSL2** with the `linux/amd64` binary. Native
