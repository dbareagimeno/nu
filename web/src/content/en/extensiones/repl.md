...
`repl` is a **Lua REPL** over the core's public API: `nu` with only `repl`
active is an interactive Lua interpreter with access to `nu.*`. It's proof
...
- A loose **expression** is evaluated and shows its value without writing
  `return` (`1 + 1` prints `2`; `nu.version.api` prints its level).
...
without pulling in the harness. In `nu.toml`:
...
- **Yields to chat.** If the official set is active (chat *and* repl), chat
  ...
  module accessible via `require("repl")`. So `nu` with the official set
  opens only chat; `nu` with only `repl` opens the REPL.
- **In headless mode** (`nu -e`, CI: no `nu.ui`) it mounts no UI; the module
...
| `repl.eval_in_task(src: string, cb: function)` | Evaluates `src` **inside a task** and delivers the result to `cb`. This is the path when user code calls the core's suspending functions (`nu.fs.read`, `nu.http.request`…), which only run inside a task. |
