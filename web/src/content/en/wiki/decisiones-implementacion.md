...
`api.md` §3 doesn't say how timers interact with the end of `nu -e`. The
...
- **`every(ms, fn)` does NOT count.** A periodic timer never ends; if it
  counted toward quiescence, `nu -e` would never return. Decision: an
  active `every` is **background convenience**, not foreground work. The
  end of `nu -e` is determined by the chunk + its tasks + its enqueued
  ...
  Rationale: in an interactive `nu` (S33+) the loop stays alive because of
  the UI/input events, not because of timers; under `nu -e` (headless, no
...
## S06 — `nu.task.future` (single-use rendez-vous, api.md §3)
...
(`Task:cancel()`, `nu.task.cleanup` with a LIFO stack, observable
...
2. **`nu.task.cleanup` (LIFO stack) during unwinding.** It doesn't exist
...
## S08 — Public cancellation: `Task:cancel` + `nu.task.cleanup` + non-catchable unwinding (api.md §1.3, §3)
...
`task.cleanups []*lua.LFunction`; `nu.task.cleanup(fn)` pushes onto it
...
methods `Task:cancel`/`nu.task.cleanup`, `ECANCELED` in `await`, and the
`pcall`/`xpcall` wrappers. New public surface = ONLY `Task:cancel` and
`nu.task.cleanup` (sacred API, §3).
...
`nu.toml`); `<=0` disables the watchdog.
...
The `nu.events` bus is S10 (doesn't exist yet). Emission is done through
...
## S10 — `nu.events` event bus (api.md §4)
...
New surface = exactly `nu.events.on/once/emit` + `Sub:cancel` (§4). The bus
...
Exact new surface: `nu.plugin.current()`, `nu.plugin.list()`,
`nu.config.dir()` [W] and `nu.config.data_dir()` [W] (§14). The canonical
...
loader**: it is NOT the Lua API `nu.toml` (that's S18, which will reuse
this same library for `nu.toml.encode/decode`). `go mod tidy` leaves
...
### Owner stack for `nu.plugin.current` and logging
...
(defer). So, DURING a plugin's `init.lua`, `nu.plugin.current()` and the
...
- **S12** (activation via `nu.toml` + `go:embed` embedded extensions): the
  ...
  hooks S12 will wire to `nu.toml`. The activation hook (which plugins get
  loaded) lives in `loader.discover`/`Boot` without pre-empting its logic.
- **S13** (`nu.plugin.reload`): relies on `ownerStack`/`currentOwner` to
...
## S12 — activation of embedded extensions governed by `nu.toml` (api.md §14, ADR-010)
...
governed by `config.dir()/nu.toml`.

### `nu.toml` is core config, not the Lua API `nu.toml`

`config.dir()/nu.toml` (config_toml.go) configures the runtime ITSELF: not
to be confused with `nu.toml`, the codec (Lua API from S18). Both reuse the
...
(forward-compat, same as `plugin.toml`). An ABSENT `nu.toml` is the
runtime's normal bare state: it activates nothing and is not an error.

### Parsed in `New`, config error DEFERRED to `Boot`

`nu.toml` is parsed in `New` (at that point `config.dir()` is already known
...
Decision: a malformed `nu.toml` is saved in `loader.configErr` and
...
`WithSliceBudget` Option > nu.toml > default**. `config.sliceBudgetSet` was
...
- if it's neither → **actionable** `EINVAL` naming the extension and the
  **`plugins.enabled` line of `nu.toml`** that fixes it (§14).
...
S11's tests (which boot without `nu.toml`). S12's test cases (embedded
gating, name-based replacement, actionable errors) are all covered.

### No new Lua surface

S12 is internal config/loader work. `nu.plugin.list()` already reflected
...
S12 left the catalog (`embeddedNames`) and `nu.toml`-based activation
...
signature is added: the only new surface is `nu.plugin.reload`, already in
§14).
...
`nu.events.once` **fires**, `dispatch` (events.go) auto-cancels it
...
doesn't freeze (other tasks make progress). **S15 (`fs.watch`), S16 (`nu.proc`) and all
...
§1.2) deliberately left out `io` and trimmed Lua's `os`; `nu.fs` is the
...
Unlike the rest of `nu.fs` (all ⏸), `watch` **does not suspend**: it arms the
...
tasks". Tasks run on the main state's event loop and share the global `nu`,
...
would hang `nu -e`. `Watcher` implements `ownedHandle` (handles.go, S13): `watch`
tags it with `currentOwner()` and `track`s it; `Watcher:stop()` `untrack`s it (no
leak in the registry) and `nu.plugin.reload` releases it via `release()` —"reload doesn't leave
...
while running the tests** and never got committed. The cause was NOT in `nu.proc` but
in a **crack in S08's cancellation unwinding** that §6's canonical idiom
(`spawn` + `nu.task.cleanup(function() p:kill() end)`) was the first to
expose.
...
The **primary** way is to kill it via `nu.task.cleanup` in whoever created it: when the
...
quiescence**: waiting for a subprocess to die for `nu -e` to return would
...
`nu.proc.alive(pid)` uses the "signal 0" trick (`kill(pid, 0)`): no error or `EPERM` (exists
...
information, like `nu.http`'s `status`). What DOES throw: a failed launch
...
**`setenv` does NOT mutate the current `nu` process's environment (the central decision, §7).**
...
Runtime (`sysState.envOver map[string]string`) that `nu.proc` applies when building
...
`nu` never changes its environment; only the child ever sees the variable.

**A lock, not the token, for the overlay.** `setenv` writes to the map in the main
state under the token, but it is **read by `nu.proc`'s background goroutines**
...
### The `nu.json.NULL` sentinel — the other 🔒 half
...
`nu.search.files` to walk the repo, but **that primitive is S27 (Phase 5) and
doesn't exist yet**. It is substituted by a **recursive walk in Lua over
`nu.fs.list`** (available since S14): enumerate the directory + recurse over
...
present as `nu.json.NULL`, distinct from nil, iterated with `pairs`; `encode` →
...
`CGO_ENABLED=1 go test -race -timeout 120s -count=2 ./internal/...` green. The `nu -e`
...
First session of **Phase 4 (Network)**. Implements **only** `nu.http.request(opts)
-> {status, headers, body}` ⏸ (§8): a **buffered** HTTP request over
S04's `suspend` bridge (ADR-011), the same pattern as `nu.fs`/`nu.proc` —the IO
...
**Global defaults in `nu.toml`'s `[net]`** (`ca_file`, `proxy`),
...
`httpState`; a malformed `nu.toml` doesn't apply it (its error is deferred to `Boot`,
...
./internal/...` green, no flakiness. The `nu -e` binary confirms end-to-end:
...
## S20 — `nu.http.stream` + SSE parser (api.md §8, 🔒)

`nu.http.stream` is the **streaming** HTTP response, the other face of
`nu.http.request` (S19, buffered). It returns a `Stream` **upon receiving the
...
is that of §6: `nu.task.cleanup(function() st:close() end)` — when the task is
...
`nu.task.cleanup(function() w:close() end)`; the safety net is
...
`nu.ws` is NOT in the 🔒 inventory (it's a wrapper over the library + the ⏸
...
regression across S01–S20). The `nu -e` binary confirms it e2e against a
...
## S22 — `nu.text` (width/wrap/truncate) + `Block` type + `nu.ui.block`/`caps`/`Style` (api.md §10, §9.2, 🔒)
...
`nu.ui.Block`) whose `__index` exposes only `.width` and `.height` (numbers)
...
- `type style struct { fg, bg string; fgSet, bgSet bool; bold, italic,
  ...
  **normalized as strings**: a `"#rrggbb"` literal (validated, lowercased) or
  ...
  (S29) degrades it against `nu.ui.caps().colors` — the renderer decides, not
  the Block (§9.2, G22).
...
### `nu.text.width/wrap/truncate` — pure CPU, [W], **none of them ⏸**
...
### `nu.ui.block`/`caps`/`Style` and the BOUNDARY NOTE (G20 is S32)

The contract says that without a TTY `nu.ui` **doesn't exist** (G20). But
that gating is S32, and S23–S31 need `nu.ui.block`/`caps`/`Style` **already**
to build and inspect Blocks in their tests (markdown/highlight/diff produce
Blocks; the theme resolves `Style`). Decision: in S22 `nu.ui` is hung up
**always** (headless too) with only `block`/`caps`; S32 will add the TTY
condition on top without touching these signatures. `nu.has("ui")` stays
**false** until S32 (a capability isn't claimed before it's actually
granted). This is explicit debt, not a contradiction of G20.

- `nu.ui.block(lines)` → Block. Each line is a **string** (an unstyled span)
...
- `nu.ui.caps()` → `{colors, kitty_keyboard, mouse, images}`. With no live
...
`CGO_ENABLED=0 go build`/`go vet`/`gofmt -l` clean; `CGO_ENABLED=1 go test
-race -timeout 120s -count=2 ./internal/...` green, no flakiness (no
regression across S01–S21). The `nu -e` binary confirms it e2e: width (5/4/2/2
...
## S23 — `nu.text.markdown` (full render, streaming-safe, themable) (api.md §10, 🔒)

`nu.text.markdown(s, opts) -> Block` renders full markdown into a `Block` of
...
(text.go) to keep all of `nu.text` in one place.
...
already extracts `lang` via `languageOf`. S24 (`nu.text.highlight`) will
...
`CGO_ENABLED=0 go build`/`go vet`/`gofmt -l` clean; `CGO_ENABLED=1 go test
-race -timeout 120s -count=2 ./internal/...` green, no flakiness (no
regression across S01–S22). The `nu -e` binary confirms it e2e: a full doc
...
## S24 — `nu.text.highlight` (syntax highlighting, degrades to plain text) (api.md §10)

`nu.text.highlight(code, lang, opts?) -> Block` highlights a snippet into a
...
and is hung from `nu.text` alongside `width`/`wrap`/`truncate`/`markdown`.
...
the `nu` module's `go` version from 1.24.7 to 1.25** (a toolchain change for
...
implements `nu.text.highlight` standalone and does NOT modify `markdown.go`,
...
`CGO_ENABLED=0 go build`/`go vet`/`gofmt -l` clean; `CGO_ENABLED=1 go test
-race -timeout 120s -count=2 ./internal/...` green, no flakiness (no
regression across S01–S23). The `nu -e` binary confirms it e2e: go (height
3), unknown → plain text (height 2), non-string `opts.theme` → `EINVAL`.

**No findings:** §10 was sufficient. Pointer ▶ advances to **S25**
(`nu.text.diff`).

## S25 — `nu.text.diff` (structured hunks + render to Block) (api.md §10, 🔒)

`nu.text.diff(a, b, opts?) -> {hunks, block?}` compares `a` (old) and `b`
...
`CGO_ENABLED=0 go build`/`go vet`/`gofmt -l` clean; `CGO_ENABLED=1 go test
-race -timeout 120s -count=2 ./internal/...` green, no flakiness (no
regression across S01–S24). The `nu -e` binary confirms it e2e: a change in
...
is; `opts` uses `render?`/`theme?`). Pointer ▶ advances to **S26** (`nu.re`).

## S26 — `nu.re` (RE2: compile/match/find_all/replace)

`nu.re` implements §10's row (`nu.re.compile(pattern) -> Re` + the `Re`
...
memory, no IO; like `nu.text` and S18's codecs): neither `suspend` nor
`requireTask`.
...
`CGO_ENABLED=0 go build`/`go vet`/`gofmt -l` clean; `CGO_ENABLED=1 go test
-race -timeout 120s -count=2 ./internal/...` green, no flakiness (no
regression on S01–S25). The `nu -e` binary confirms e2e: match with groups
...
were implemented as-is). Pointer ▶ advances to **S27** (`nu.search.files`).

## S27 — `nu.search` (files/grep/fuzzy) (api.md §11, 🔒; closes Phase 5 — CP-6)
...
- **Cancellation (S08):** when the iterator is created, an
  `nu.task.cleanup` is registered (`registerGrepCleanup`, manipulates the
...
data, like `nu.re`/codecs — it does not use the `suspend` bridge.
...
`CGO_ENABLED=0 go build`/`go vet`/`gofmt -l` clean; `CGO_ENABLED=1 go test
-race -timeout 120s -count=2 ./internal/...` green, no flakiness (no
regression on S01–S26). The `nu -e` binary confirms e2e. **APILevel stays
at 1.**
...
`nu.ui` (cells, regions, blit, diff→ANSI), but it is **not hung off** the
`nu` global: it lives in `internal/runtime/spike_compositor.go` (the
...
bothers production, the fix is in `nu.search.fuzzy` (parallelize the
...
- **Headless `size()`:** with a real TTY it reads from the terminal;
  without a TTY an injectable default for tests (gating "nu.ui does not
...
(api.md not extended, `nu.version.api` stays at 1; **no `G##` findings**).
...
- **resize preserves content in the top-left corner.** §9.1 leaves open
  ...
  `w/h<0` → `EINVAL`, same as `nu.ui.region`.
...
- **Sequence resolution = pending buffer + generation + `oneShot`.** The
  ...
  did not reuse `nu.task.every` (it is periodic and its `fn` is Lua) nor an
  `nu.fs` ⏸ (input dispatch is synchronous, not a task): the timeout
...
- **G30 (image paste): SYNCHRONOUS dump with a direct Go write.** §9.3/G30:
  a paste of non-text content is dumped to `nu.fs.tmpdir` and delivered as
  ...
  (under the token, NOT in a ⏸ task), so the dump cannot be an `nu.fs.write`
  ⏸. I resolved this with `writePasteImage`: reuses `fs.ensureTmpdir` (the
  machinery of `nu.fs.tmpdir`) and a direct Go `os.WriteFile`
...
## S32 — rest of `nu.ui`: OSC 52 clipboard + `ui:*` events + headless gating G20 (api.md §9.2, §9, §4, §2)

### HEADLESS GATING (G20, the central decision)

§9/G20: without an interactive TTY (`nu -e`, CI, redirected output) the
`nu.ui` module **flatly DOES NOT EXIST**, and detection is `nu.has("ui")`
...
- **`registerUI` is called only if `rt.uiActive`** (in `registerNu`). In
  headless mode `nu.ui` is neither hung off the global nor is the
...
- **`uiActive` is decided by `New`:** `WithForceUI(active)` overrides
  (precedence, `forceUISet`); without it, `detectTTY()`. So the `nu` binary
...
- **`nu.has` becomes per-runtime** (`rt.caps()`), not a global map:
...
Tests run headless (no TTY), so with real gating `nu.ui` would not exist
...
`nu.events` and leaves the paths:
...
`api.md` was NOT touched, nor `nu.version.api` (APILevel stays at 1): the
`clipboard_set`/`clipboard_get` signatures, the `ui:*` events, and the
gating were already specified. Per-runtime `nu.has` is implementation, it
does not change the `nu.has(cap) -> boolean` signature. No `G##` findings.
...
capabilities: version + API level (`nu.version`), config and plugin paths
...
TTY (`nu` without `-e` in CI) → `BareScreenActive` is false → usage is
printed (starts bare), confirmed with the binary.

### Actions: activate → write nu.toml → continue Boot (no network)

`activateAndBoot(names)` writes `names` into `plugins.enabled` of
`config.dir()/nu.toml`, reloads the config in the loader, resets `booted`,
...
### Writing nu.toml: preserve the rest of the file, atomic

`writeEnabledPlugins` reads the existing `nu.toml` into a generic
...
half-written `nu.toml`). A MALFORMED `nu.toml` is **not blindly
...
- **Content / FIXED render to buffer**: the model and the **compositor
  ...
  frame is not empty.
- **Activate official set → nu.toml → Boot**: writes `plugins.enabled` with
...
- **Preserve config**: writing `enabled` preserves `dirs`, `watchdog`, and
  unrelated keys/sections; the result is a valid `nu.toml`.
- **Malformed nu.toml** is not overwritten (EINVAL, file intact).
- **No regression**: headless `nu -e` still works; `nu` without `-e` and no
  TTY prints usage.
...
paste. The screen render, the condition, and the activate→nu.toml→Boot
...
## S34 — `nu.worker.spawn` + caps (G6) + send/recv with bounded queues (api.md §13, 🔒; opens Phase 7)

First session of Phase 7 (Workers). Implements **opt-in parallelism**
(ADR-008): `nu.worker.spawn` spins up a NEW, isolated Lua state in its own
...
pattern is an `nu.worker.parent.recv()` (⏸) loop. That is why `run` does
`s.spawn(require(module))` and then `waitIdle`. A worker is always headless
(`uiActive=false`, `ui=nil`): no `nu.ui`.

### Caps filtering (G6): deny-by-default, two granularities

`registerWorkerNu` registers the WHOLE [W] surface in the worker's `nu` by
...
same model as `nu.ui`'s gating (G20): it does not "throw EACCES," the
...
[W] (never reach the worker): `nu.ui`, `nu.events`, `nu.fs.watch`,
`nu.worker.spawn` (no nesting), nor `nu.plugin` (§16). `nu.version`/
`nu.has` and `nu.worker.parent` always go through (they are not prunable
surface: capability detection and the channel with the parent).
`nu.config.dir`/`data_dir` are [W] (§14).
...
and `nu.worker.parent.send` (worker) are ⏸: they enqueue by SUSPENDING if the queue is full
...
`nu.json.NULL` sentinel is per-state userdata and could not cross either way). This is where
...
`nu.task.sleep`/`http`/`proc`/`await`/... did NOT wake up, so `driveUntilDone`→`waitIdle` blocked
...
- **immediate, leak-free `terminate` (review)** (`TestWorkerTerminateInterruptsSleep`): a worker
  suspended in `nu.task.sleep(60000)` is cut off on the spot by `terminate()` —`terminate`+`Close`
...
no new piece, mirrors `nu.task.every`'s ticker (timers.go):
...
NOT the `Sub` from `nu.events` (carries `*subscriber`): same public method `:cancel()`, different
...
`nu.task` [W]. S35 SHIELDS it with a test (`TestWorkerInternalTasksTimersFutures`): a worker runs
...
(walks it with `nu.search.files`, reads with `nu.fs.read`) and returns a digest to the
principal via `send`/`recv`; INSIDE the worker `nu.fs.write` and `nu.ui` do NOT exist
...
**Why a `require`-able module and not a `nu.providers` namespace:** the core reserves the `nu`
...
1. **`providers.toml` is read lazily, which is why `resolve`/`list` SUSPEND (⏸).**
   providers.md §1 says it "lives in `nu.config.dir()`" but does not fix *when* it is read. It is
   read with `nu.fs.read` (⏸, api.md §5) the first time anyone resolves or lists, and is cached;
   `reload()` invalidates the cache. A consequence inherited from the API: since `nu.fs.read`
...
4. **`api_key` ALWAYS from the environment** (providers.md §1: "never the key in the file"). It
   is read with `nu.sys.env(prov.api_key_env)`. If the provider does not declare `api_key_env`
...
`nu.toml.decode` **was stringifying arrays-of-tables** (`[[providers.x.models]]`), the CENTRAL
...
**It is not a gap in api.md**: the documented signature `nu.toml.decode -> v` already promised
...
`input_json_delta.partial_json`; I accumulate it as text and decode it with `nu.json.decode` WHEN
...
consumes the SSE via `nu.http.stream` → emits the canonical stream → for each text delta I
recompose the accumulated markdown with `nu.text.markdown` (streaming-safe, S23) and blit it to a
...
environment `nu.ui` does not exist (G20 gating). To exercise the hot path's blit, `bootAnthropic`
...
None. The public API was exactly enough (`nu.http.stream`+`Stream:events()` §8, `nu.json` §12,
`nu.text.markdown` §10, `nu.ui.region`/`Region:blit` §9). api.md INTACT; APILevel stays at 1.
Pointer ▶ advances to **S38** (sessions extension; depends on S14, S16).

## S38 — Sessions extension (JSONL, lockfiles) + nu.sys.pid (G32, APILevel 1→2)
...
not expose it: `nu.sys` provided platform/env/setenv/now_ms/mono_ms/hostname and
`nu.proc.alive(pid)` validates OTHER processes' pids, not one's own. It is the loose end of G17
...
`nu.sys.pid() -> integer` [W] (not ⏸; a local query with no IO, like `hostname`/`now_ms`), a
wrapper around `os.Getpid()`. Since it is the FIRST addition after the freeze, `nu.version.api`
goes from 1 to 2 (api.md §17/§2; `APILevel` in `nu.go`). G32 RESOLVED in `problemas.md`; api.md
§7 and §16 updated; `sesiones.md §6` uses `nu.sys.pid()`. It is a strict addition: it does not
...
- **Lockfile** (§6, G5): `<session>.jsonl.lock` with `fs.write{exclusive}`; contents
  `{pid=nu.sys.pid(), hostname, started}`. Conflicts resolved by inspection: same hostname + dead
  pid (`proc.alive`=false) → orphan, silently reclaimed; live pid → ESESSION busy; different
  hostname → ESESSION foreign (not verifiable remotely). Released by `nu.task.cleanup`.
...
activatable via `nu.toml` `plugins.enabled=["providers","sessions","agent"]` (all three explicit:
...
**Does NOT expand api.md** (completeness corollary satisfied): `nu.events` (§4),
`nu.task.future`/`spawn` (§3), `nu.has("ui")` (§9, G20), `nu.fs`/`nu.toml`/`nu.config.dir`, and
...
granted (explicit and noisy, buffer 3); if `mode="ask"` AND `nu.has("ui")` → emits
...
**`agent:*` events (agente.md §4, notifications via `nu.events`):** session.start/end,
...
- **`requires` does not auto-activate**: the test `nu.toml` enumerates all three extensions;
...
- **System prompt (§7) partial in S39**: only base + `opts.system`. The skills index (§6), the
  repo's `nu.md`, and TOFU/trust (§11) are later work (not in S39's scope).
...
HEADLESS (`WithForceUI(false)`, `nu.has("ui")`=false), runs a turn with the real file tool
`read_file` (reads a file from disk with `nu.fs`, granted since it's read-only, its content is
...
**Tests** (`agent_test.go`, S12's harness with all three extensions via `nu.toml`): load+
...
principal sharing tools; `worker=true`: a loop in an `nu.worker` with trimmed `caps` and tool
handlers executed in the principal via a message proxy), plus the named caps bundles
`agent.caps.*`. All Lua over the frozen API (ADR-003): `nu.worker` §13 + `nu.task` + the
...
- `lua/agent/subagent_worker.lua`: the subagent's LOOP that runs INSIDE the worker. It is the
  `module` that `nu.worker.spawn("agent.subagent_worker", {caps=...})` loads.
...
2. **The tool proxy** (worker mode). The worker does NOT execute handlers: for every tool_call
   it sends `{kind="tool_call", id, name, args}` to the parent via `nu.worker.parent.send` and
...
4. **Default caps of a subagent-worker: read-only.** `FS_RO` (fs.read/stat/list/cwd) +
   ...
   with `nu.fs.read`+`nu.toml.decode` from `nu.config.dir`—. Without them the worker could not
...
The subagent-worker is expressed entirely with the public API: `nu.worker.spawn` with `caps`
(api.md §13, G6) for HARD isolation; `Worker:send`/`recv` + `nu.worker.parent.send`/`recv` for
the init/tool_call/tool_result/done protocol (JSON-able messages copied); `nu.task` for the
...
Inside the worker there is neither `nu.events` (main bus) nor `nu.ui` (api.md §16). The
...
`lua/mcp/init.lua` module. INACTIVE by default (ADR-010); enabled via `nu.toml`
...
`mcp.connect{ name, command, cwd?, env? } ⏸ -> Conn` launches the server with `nu.proc.spawn`
(S16) and talks to it over stdin (JSON requests via `nu.json.encode` + `Proc:write`), reading
responses from stdout line by line (`Proc:read_line` + `nu.json.decode`). Demultiplexing: a
**dedicated reader task** (`dispatch_loop`) reads stdout and dispatches each response to its
pending request by `id` (each `request` registers an `nu.task.future` that the reader resolves),
...
registered in `nu.task.cleanup` (dies when the owning task ends) and an explicit, idempotent
...
`nu.fs`/`nu.toml`/`nu.config.dir` + the `agent` module (`agent.tool`, `agent.tools`) were EXACTLY
...
- **Themes (G22)** (`toolkit.theme`): THIS is the point of G22. The core only understands literal
  ...
  mapped "accent" to another name would fail later inside `nu.ui.block`; validating it at
...
- **label**: a single line of styled text (statusline, headers). Not focusable. `pref_h=1` by
  default (a label occupies its row, not 0). Composes with `nu.ui.block` + `theme:style`.
- **text**: multiline markdown block (`nu.text.markdown`, streaming-safe) or word-wrap
  (`nu.text.wrap`), with viewport SCROLL. Composes the COMPLETE Block; scroll is an offset
...
The toolkit was built EXACTLY on top of API §9 (`nu.ui.region`/`blit`/`fill`/`clear`/`cursor`/
`size`, `nu.ui.block`/`Style`, `nu.ui.on_input`) + §10 (`nu.text.markdown`/`wrap`/`truncate`/
`width`) + §4 (`nu.events.emit`, with its own `toolkit:focus` event in the plugin's namespace) +
§2 (`nu.has`). Not one extra public function; APILevel stays at 2. No `G##` findings. Confirms
...
headless mode `nu.ui` doesn't exist, G20 — the Block is opaque to Lua, so its CONTENT is
...
band prevents it from spilling onto the one below); `app` without `nu.ui`→EINVAL.
`CGO_ENABLED=0 go build`/`go vet ./...` green; `gofmt -l` clean; `CGO_ENABLED=1 go test -race
...
full suite; it is ORTHOGONAL to S42 (the toolkit is Lua over `nu.ui`/`nu.text`, doesn't touch
...
(ADR-010), enabled via `nu.toml` `plugins.enabled=[...,"chat"]`, `source="builtin"`. Implements
...
(`nu.ui.size()`), routes input to the focused widget (the editor), and repaints via dirty nodes
...
chat hardcodes NOT a single color: the transcript's final render is done by `nu.text.markdown`
...
`chat.start(opts?) ⏸ -> Chat` requires `nu.ui` (interactive TTY): in headless mode (no UI,
`nu.has("ui")`=false, G20) it's an **actionable EINVAL** (naming "headless"/"ui"; chat.md §8 —
...
headless mode it leaves the module accessible via `require("chat")` without touching `nu.ui`.
Subscribes to `ui:resize` ("your region, your ui:resize", api.md §9.1): redoes layout on resize.

### Does NOT extend api.md (completeness corollary satisfied)

`nu.events.on`/`emit` (§4), `nu.ui` (§9, via the toolkit), `nu.text.markdown` (§10, via the
toolkit), `nu.task.spawn`/`sleep`/`future` (§4), `nu.has("ui")` (§2/§9), `nu.ui.keymap` (§9.3),
`nu.json` (§12, in the dialog) plus the `toolkit`/`agent`/`providers`/`sessions` modules were
...
### Tests (`internal/runtime/chat_test.go`, S12 harness with the five extensions via `nu.toml`, `WithForceUI(true)`+`WithUISize`)
...
2. **Fuzzy pickers (chat.md §3 `@` mentions, §4 `/model`/`/sessions`) by text in v1**: the visual
   picker (a `toolkit.stack` modal layer + `nu.search.fuzzy`) is a later improvement; the
...
interactive UI if it has one, and the TTY startup pattern (`nu.has("ui")`, subscription to
...
mode (`nu -e`, `--auto-permissions`). The `agent:*` event contract that chat consumes is stable
...
INACTIVE by default (ADR-010), enabled via `nu.toml` `plugins.enabled=["repl"]`,
`source="builtin"`.
...
S01) disabled blocking IO. Is the public API enough, or is a primitive missing (`nu.eval`/
...
- **Conclusion: the public API is EXACTLY enough for a REPL. NO new primitive was needed (neither
  `nu.eval` nor reopening `load` in a controlled way); APILevel stays at 2, api.md UNCHANGED.**
  ...
  and report it" protocol (in the style of `nu.sys.pid` in S38) did NOT trigger because the
...
1. **return<expr> / statement trick** (`compile`): first `loadstring("return "..src)` — so a
   bare EXPRESSION (`1+1`, `nu.version.api`) is evaluated without the user writing `return`, the
...
`cb`: this way a line calling a **⏸** core function (`nu.fs.read`, `nu.http.request`,
`nu.search.grep`…) works — they only run inside a task, §1.3. It's the same pattern as
`chat:submit` (S43) with `Session:send`. Most of the API (non-⏸: `nu.version`, `nu.text.*`,
`nu.json.*`) doesn't need it and goes through `repl.eval` directly.
...
- **TTY DRIVER (`repl.start`, needs `nu.ui`, G20)**: mounts a `toolkit.app` (S42) with a
...
`nu.task.spawn` §3 (eval_in_task) + `nu.version`/`nu.has` §2 + the `toolkit` module (the UI) +
`nu.ui.keymap`/`nu.events.on` §9.3/§4 were EXACTLY enough for a standalone-activatable
...
2. **The output format** (a string with `%q`, returns joined by tab, `code: message` for the
   ...
   vocabulary (how a REPL PRINTS), not the core's. A faithful serializer is `nu.json.encode`, which the
...
S45 = **CLI surface** (open question #5 of architecture): `nu -e` flags,
...
(`nu -e` already evaluates via `EvalString`/Go; the repl is its interactive counterpart). The
"standalone-activatable on TTY vs. headless logic" pattern (G20/G21) that the CLI replicates for `nu -e`. With S45
...
**What it is.** The LAST link of the plan: the command-line surface of the `nu` binary.
Closes open question #5 of [arquitectura.md](arquitectura.md), Phase 8, and the entire
plan (45/45). It lives in `main.go` (the binary), **NOT in the sacred `nu.*` API** (api.md): it's the
...
into the kernel—; (b) CP-10's two-phase pattern (`EvalString` that `nu.task.spawn`s + globals +
...
silent new session. The project is `nu.fs.cwd()` (where `nu` was launched).
...
would fail intermittently with `local denied = false; ... denied = true`. **Cause:** `nu.events`
...
dirs and headless. `nu -e` success (0) + structured error (1, stderr with code, nothing on stdout) +
...
counterpart `TestUnhandledTaskErrorLogged` (fire-and-forget via `nu.task.spawn` through `EvalString`,
