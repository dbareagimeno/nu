...
The entire API lives under the global `nu`. `require` is reserved for
...
`os.getenv`, `print` (redirected to `nu.log.info`), `dofile`/`loadfile`
...
- **Synchronous handlers** (input, events) run on the loop and cannot call
  ⏸ functions; to do IO, they spawn a task with `nu.task.spawn`.
- **Watchdog**: every continuous Lua execution *slice* (between two
  ...
  `nu.toml`). Exceeding it aborts the task and emits
...
- **Cancellation and aborts are NOT catchable.** `Task:cancel()` and the
  ...
  **unwinding the stack without going through `pcall`** — if they were
  ...
  matter what, register `nu.task.cleanup(fn)`. `ECANCELED` is reserved for
...
`nu.proc.run`, `nu.http.request`, `nu.http.stream`, `nu.ws.connect`—; the
...
| Signature | Semantics |
|---|---|
| `nu.version -> {major, minor, patch, api: integer}` [W] | Runtime version and API level. |
| `nu.has(cap: string) -> boolean` [W] | Capability detection (`"ui"`, `"ui.images"`, `"net.tcp"`, ...) for portable extensions. Also covers entire modules: in headless mode `nu.ui` doesn't exist (§9). |
...
| Signature | Semantics |
|---|---|
| `nu.task.spawn(fn, ...) -> Task` | Spawns a task; extra arguments are passed to `fn`. |
| `nu.task.sleep(ms)` ⏸ | Suspends the current task. |
| `nu.task.all(fns: Task[]\|fn[]) -> any[]` ⏸ | Waits for all of them; if one throws, it cancels the rest and rethrows. Results are returned **aligned with the inputs** (`out[i]` is the one for `fns[i]`), never in completion order (G27) — this is what lets you correlate result with input in a fan-out without hand-carrying the index. |
| `nu.task.race(fns) -> (winner_index, result)` ⏸ | First to finish wins; cancels the rest. |
| `nu.task.every(ms, fn) -> Timer` | Periodic timer (synchronous handler). `Timer:stop()`. |
| `nu.task.defer(fn)` | Runs `fn` on the next loop tick. |
| `nu.task.future() -> Future` | Single-use rendez-vous: `Future:set(v)` (synchronous, one time only; later calls throw `EINVAL`) and `Future:await() -> v` ⏸ (several can wait; if already resolved, returns immediately). This is the piece for "one task waits for a value some other code will produce" (dialogs, pickers, proxies) without polling. |
| `Task:cancel()` | Cooperative cancellation: aborts the task at its next suspension point (not catchable, §1.3); its `cleanup`s run. |
| `nu.task.cleanup(fn)` [W] | Registers a (synchronous) releaser on the current task's LIFO stack; all of them run on termination — success, error, or abort. The `defer` of this house: processes, regions, input handlers. |
| `Task:await() -> any` ⏸ | Waits for another task's result. |

---

## 4. `nu.events` — event bus
...
| Signature | Semantics |
|---|---|
| `nu.events.on(name, fn) -> Sub` | Subscribes. Synchronous handlers, in registration order, each under `pcall` (ADR-008). `Sub:cancel()`. |
| `nu.events.once(name, fn) -> Sub` | Once only. |
| `nu.events.emit(name, payload?)` | Synchronous dispatch on main state. |
...
| Signature | Semantics |
|---|---|
| `nu.fs.read(path) -> string` ⏸ | Reads the entire file. |
| `nu.fs.write(path, data, opts?)` ⏸ / `nu.fs.append(path, data)` ⏸ | Atomic write (write via temp file + rename). `opts.exclusive = true` (G17): creates **only if it doesn't exist**, in a single indivisible operation (`O_EXCL` — there's no temp+rename here: rename would overwrite); if the file already exists it throws `EEXIST`. This is the piece for lockfiles ([sesiones.md](sesiones.md) §6). |
| `nu.fs.stat(path) -> {size, mtime_ms, is_dir, mode}?` ⏸ | `nil` if it doesn't exist (doesn't throw `ENOENT`). |
| `nu.fs.list(dir) -> {name, is_dir}[]` ⏸ | Non-recursive; for recursive see `nu.search.files`. |
| `nu.fs.mkdir(path)` ⏸ / `nu.fs.remove(path, opts?)` ⏸ / `nu.fs.rename(from, to)` ⏸ / `nu.fs.copy(from, to)` ⏸ | `remove` requires `opts.recursive=true` for non-empty directories. |
| `nu.fs.tmpdir() -> string` ⏸ | The session's own temporary directory. |
| `nu.fs.cwd() -> string` [W] | Working directory (immutable during the session; subprocesses can receive a different one via `opts.cwd`). |
...
## 6. `nu.proc` — subprocesses [W]

| Signature | Semantics |
|---|---|
| `nu.proc.run(argv: string[], opts?) -> {code, stdout, stderr}` ⏸ | Convenience with buffers. `opts`: `cwd`, `env`, `stdin`, `timeout_ms`. No implicit shell: `argv` is an array; whoever wants a shell invokes it explicitly. |
| `nu.proc.spawn(argv, opts?) -> Proc` | Fine-grained control with streams. |
...
| `nu.proc.alive(pid: integer) -> boolean` | Is there a live process with that `pid` on this machine? (G17). Reports **existence, not identity** — a recycled pid gives `true`. For detecting orphaned locks ([sesiones.md](sesiones.md) §6). |

Process lifetime: the rule is to kill it explicitly via `nu.task.cleanup`
...
## 7. `nu.sys` — environment and clock [W]

| Signature | Semantics |
|---|---|
| `nu.sys.platform() -> "linux"\|"darwin"\|"windows"` | |
| `nu.sys.env(name) -> string?` / `nu.sys.setenv(name, value)` | `setenv` only affects future subprocesses. |
| `nu.sys.now_ms() -> number` / `nu.sys.mono_ms() -> number` | Wall clock / monotonic clock. |
| `nu.sys.hostname() -> string` | Machine name (G17; content of session locks, [sesiones.md](sesiones.md) §6). |
| `nu.sys.pid() -> integer` | Pid of the current `nu` process (local query, like `hostname`/`now_ms`). Together with `hostname` it forms the **writer identity** of session locks (G32; [sesiones.md](sesiones.md) §6). Different from `nu.proc.alive(pid)`, which validates *other* pids: `pid()` is your *own*. |

---

## 8. `nu.http` and `nu.ws` — networking [W]
...
| Signature | Semantics |
|---|---|
| `nu.http.request(opts) -> {status, headers, body}` ⏸ | `opts`: `url`, `method?`, `headers?`, `body?`, `timeout_ms?`, `tls?`, `proxy?` (per-request TLS/proxy, see note G12 below). Buffered response. Doesn't throw on status >= 400 (status is data); throws `ENET`/`ETIMEOUT` on transport failures. |
| `nu.http.stream(opts) -> Stream` ⏸ | Returns upon receiving headers: `Stream.status`, `Stream.headers`. `opts.timeout_ms` covers up to the headers; `opts.idle_timeout_ms?` throws `ETIMEOUT` if N ms pass without receiving body bytes (an SSE can go silent forever). |
...
**Headless (G20)**: without an interactive TTY (`nu -e`, CI, redirected
output), the `nu.ui` module simply **doesn't exist** — the same model as
worker `caps`: ungranted surface isn't there. Detection is `nu.has("ui")`,
never try-and-catch.
...
| Signature | Semantics |
|---|---|
| `nu.ui.size() -> {w, h}` | Terminal size in cells. Changes → `ui:resize` event. |
| `nu.ui.region(opts) -> Region` | `opts`: `x, y, w, h, z?`. Regions are the unit of composition: rectangles with z-order owned by whoever creates them. **Resize (G1)**: a region fully or partially off-screen gets clipped without error (it never paints out of bounds; if nothing fits, nothing is painted); its coordinates aren't touched — if the screen grows again, it reappears as-is. Repositioning is the owner's responsibility (convention "your region, your `ui:resize`"); automatic relayout is the toolkit's job, not the core's. |
| `Region:blit(x, y, block: Block)` | Stamps a pre-rendered block (see `nu.text`) at local coordinates of the region. **Clips on both ends (G28)**: `x/y` can be **negative** and clip the block's *starting* edge (`blit(0, -3, doc)` shows `doc` starting from its fourth row), just as excess clips the end — a **viewport** over a Block bigger than the region, where *scroll = re-blit with a different offset*. It's **copy, never re-render**: blitting the same Block with a different offset recomputes nothing (the cost of scrolling is that of a copy of the visible window). Virtualization (not building the entire Block for huge histories) belongs to the toolkit, not the core. |
...
A **Block** is an opaque handle to styled lines, produced by `nu.text.*`
or built by hand. It has `.width` and `.height`.

| Signature | Semantics |
|---|---|
| `nu.ui.block(lines: (string\|Span[])[]) -> Block` | Manual construction. A `Span` is `{text, style?}`. |
| `Style` | Table `{fg?, bg?, bold?, italic?, underline?, reverse?}`; **literal** colors: `"#rrggbb"` or index 0-255 (rendering degrades them to whatever the terminal supports, `nu.ui.caps().colors`). Semantic names (`"accent"`, `"error"`, ...) **aren't the core's**: they're vocabulary of the toolkit's theme, which resolves them to literals when building Blocks (G22). |
| `nu.ui.caps() -> {colors, kitty_keyboard, mouse, images}` | Terminal capabilities. |
| `nu.ui.clipboard_set(s)` / `nu.ui.clipboard_get() -> string?` ⏸ | Via OSC 52 when the terminal supports it. |
...
| Signature | Semantics |
|---|---|
| `nu.ui.on_input(fn) -> InputHandle` | Pushes a synchronous handler `fn(ev) -> boolean` (true = consumed) onto the stack. `ev`: `{type: "key"\|"mouse"\|"paste", key?, mods?, x?, y?, text?, path?}`. `InputHandle:pop()`. |
...
(`nu.fs.tmpdir`) and delivers the `paste` event with `path` (the dumped
...
## 10. `nu.text` — rendering and processing [W]

Quadratic-on-screen operations live here, in Go (ADR-004/007).

| Signature | Semantics |
|---|---|
| `nu.text.width(s) -> integer` | Width in cells (graphemes, east-asian, emoji). |
| `nu.text.wrap(s, width, opts?) -> Block` | Word-wrap; `opts.style?` (a Style §9.2) applies to each line. |
| `nu.text.truncate(s, width, opts?) -> string` | With optional ellipsis. |
| `nu.text.markdown(s, opts) -> Block` | Full markdown rendering at `opts.width`, themable. Accepts incomplete input (streaming-safe). |
| `nu.text.highlight(code, lang, opts?) -> Block` | Syntax highlighting. |
| `nu.text.diff(a, b, opts?) -> {hunks, block?}` | Structured diff; `opts.render=true` additionally returns the painted Block. |
| `nu.re.compile(pattern) -> Re` | RE2 regex. `Re:match(s) -> caps?`, `Re:find_all(s) -> ranges`, `Re:replace(s, repl) -> string`. |
...
## 11. `nu.search` — repo-scale search [W]

| Signature | Semantics |
|---|---|
| `nu.search.files(root, opts?) -> string[]` ⏸ | Recursive listing respecting `.gitignore`. `opts`: `glob`, `hidden`, `max`. |
| `nu.search.grep(pattern, opts) -> iterator` ⏸ | Parallel internally; iterates `{path, line_no, line, ranges}` as they arrive. `opts`: `root`, `glob`, `case`, `max`. |
| `nu.search.fuzzy(query, candidates: string[], opts?) -> {index, score}[]` | Ordered fuzzy matching, for pickers. Synchronous and bounded (it's the picker's hot primitive). |
...
| Signature | Semantics |
|---|---|
| `nu.json.encode(v, opts?) -> string` / `nu.json.decode(s) -> v` | `opts.pretty`. `null` ↔ `nu.json.NULL` (sentinel) so keys aren't lost. **Strict about UTF-8** (G11): `encode` throws `EINVAL` on invalid bytes — sanitizing is a visible decision made by whoever has context (the tool), never the codec. |
| `nu.toml.encode(v) -> string` / `nu.toml.decode(s) -> v` | |
| `nu.yaml.encode(v) -> string` / `nu.yaml.decode(s) -> v` | Needed for metadata from the existing ecosystem (skill frontmatter); YAML is too treacherous to parse in pure Lua. |

---

## 13. `nu.worker` — opt-in parallelism (ADR-008)

| Signature | Semantics |
|---|---|
| `nu.worker.spawn(module: string, opts?) -> Worker` | Spins up a new Lua state in its own goroutine, loading `module` (resolvable by the loader). The loader's `require` paths (plugin Lua modules) are available inside the worker; what doesn't exist is the `nu.plugin` API (lifecycle). No `nu.ui`, no `nu.events` (main bus), no nested workers. `opts.caps?: string[]` restricts the worker's API to what's listed, with **two granularities** (G6): `"fs"` grants the entire module; `"fs.read"` grants a specific function. What's not granted **doesn't exist** inside the state — capability-based sandboxing; functions added to the API in the future are never granted by old lists (deny-by-default for new surface). Without `caps`, the worker gets the entire [W] API. Named packages (e.g. read-only): tables from the agent extension (`agent.caps.*`), not the core. |
...
| *(inside the worker)* `nu.worker.parent.send(msg)` ⏸ / `...recv() -> msg` ⏸ | Channel with main state; same bounded queues. |

Inside a worker (G15): each worker is a **complete mini-runtime** — its own
scheduler, multiple tasks, timers, and futures (all `nu.task` [W]). **No
watchdog**: workers exist precisely to burn CPU freely; control is
`terminate()` from the main state plus `caps`.

---

## 14. `nu.plugin` and the loader
...
**Runtime configuration**: `config.dir()/nu.toml` governs the core
...
`nu.toml` line that fixes it, like the permission errors in
...
set to `nu.toml` —and active `agent.toml`/`providers.toml` templates if
...
nothing to `nu.*` and doesn't move `nu.version.api`.
...
| Signature | Semantics |
|---|---|
| `nu.log.debug/info/warn/error(fmt, ...)` | To a file in `data_dir`, annotated with the originating plugin. `print` is an alias for `info`. Never to the screen: UI belongs to extensions. |
...
- Freezing v1 = freezing **this document**: signatures and semantics only
  change by addition; `nu.version.api` increments with each addition.
  **Current level: `api = 3`** — level 1 was the initial freeze;
  `nu.sys.pid()` (G32) bumped it to 2; `nu.ws`'s binary frames (G52:
...
- Capability detection with `nu.has()`, never version sniffing.
