...
| 🔒 **G65 (kernel)** | Parser dual de `opts.env` en `enu.proc.run`/`spawn` (`parseProcArgsWasm`, api.md §6; sube `APILevel` a 6): las dos formas puras —tabla `{K=V}` y array `["K=V"]`— llegan al hijo; el valor puede contener `=` (split por el PRIMERO); entre claves repetidas del array gana la ÚLTIMA; `env` presente aunque vacío **reemplaza** (la tabla Lua `{}` cruza el wire como array vacío) y ausente hereda; TODO env malformado → `EINVAL`, nunca silencio — incluidos los bordes simétricos clave-vacía en tabla (`{[""]="x"}`) y en array (`{"=x"}`, la mitad `eq == 0` de la guardia) y la entrada sin `=`; un env array explícito pisa el overlay de `setenv` (`vmwasm_proc_test.go`). En mcp, `normalize_env` devuelve `nil` ante un env declarado VACÍO (vacío = heredar, no reemplazar-con-vacío: un `env = []` en `mcp.toml` no mata al servidor por perder `PATH`) — test propio del borde. |
