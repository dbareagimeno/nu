...
cualquier capacidad nueva; su superficie vive en el binario (`main.go` y el
driver de TTY), NO en `api.md` (que queda intacto). La primera la levantó el
operador al notar que faltaba el afijo de versión más convencional; la segunda,
al notar que la pantalla de runtime desnudo pinta acciones que no responden al
teclado.
...
| **S54** | **Elección por teclado en la pantalla de runtime desnudo** (completa G21/ADR-010: la activación «de una tecla» que S33 dejó explícitamente pendiente — la pantalla pinta las tres acciones de [api.md](../contracts/api.md) §14 pero hoy solo responde a salir). Un handler de input del kernel (instalado por `PrepareBareScreen`, sobre la pila pública `on_input`, como el de salida) mapea: **`1`** → activar el **conjunto oficial de producto** vía `ActivateOfficial()` (exactamente `officialProductSet`, ADR-015: «la pantalla desnuda y el flag activan lo mismo»; `example` fuera); **`2`** → modo selección sobre el catálogo de embebidas que la pantalla ya lista (cursor `↑`/`↓` o `j`/`k`, `enter` activa **solo** la elegida vía `activateAndBoot`, `esc` vuelve al menú — el mínimo que hace expresable la acción «activar sueltas» ya congelada; el estado del cursor vive en `bare_screen.go`/driver, **jamás** como widget de `enu.ui`, y generalizar el menú sería hallazgo, no código); **`3`**/`q`/`esc`/`ctrl+c` → salir (ya cableado). Tras activar, la UI de producto se monta en el **mismo proceso** (la cadena activar→`enu.toml`→`Boot` de S33, sin red). Errores en pantalla, no al vacío: un `enu.toml` mal formado (`EINVAL`, no se pisa) o un `Boot` que falla tras activar pintan el error accionable en la propia pantalla y dejan viva la salida por teclado (ADR-017: ninguna ruta interactiva atrapa la terminal). **NO amplía `api.md`** (`enu.version.api` quieto): es interfaz del binario (driver de TTY), el mismo estatus que el cierre de CP-7. Juez de filosofía: VÍA LIBRE (2026-07-19); una **re-revisión independiente** (auditoría del feedback 2026-07-19) confirmó el fondo y pidió dos reparos de coherencia, aplicados en `api.md` §14 **antes** de construir S54: (1) §14 describía la pantalla como «sin lógica», que no cuadra con esta máquina de estados de selección — ahora dice que responde al teclado con una selección acotada cuyo estado vive en el driver, nunca como widget de `enu.ui`; (2) el freno «generalizar el menú es hallazgo, no código» pasa de nota del plan a **cláusula de §14** (contrato sagrado), el freno más duro posible contra que el selector derive en una TUI del kernel. | S31 (pila de input), S33 (pantalla + activación), driver de TTY (CP-7) | [api.md](../contracts/api.md) §14 (las tres acciones y su semántica) · [G21](../findings/g21-el-primer-arranque-de-adr.md) · ADR-010 («una tecla») · ADR-015 · ADR-017 | En un TTY sin plugins: `1` escribe `plugins.enabled` con el conjunto oficial y el producto se monta en el mismo proceso; `2` activa **solo** la embebida elegida; salir sigue funcionando; con `enu.toml` mal formado el error accionable se pinta en pantalla, el fichero queda intacto y `q` sigue saliendo; una segunda pulsación de activar con el Boot en curso no dispara otro. Con esto, CP-7 manual queda por fin ejecutable completo. |
