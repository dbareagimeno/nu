...
1. **El puntero** (esta misma línea, la única imperativa del documento):
   ...
   > **▶ Próxima sesión: `S07`** · Fase 1 · S06 cerrada (`nu.task.future`: `set` síncrono una sola vez, `await` múltiple). El scheduler sigue realizándose **sin yields de corrutina** (goroutine-por-task + token Lua, **ADR-011**, grieta **G31**). S07 (`task.all`/`race`) depende de S04 **y S06** (ambas cerradas) y se construye sobre ese modelo; está en el inventario 🔒 (`out[i]` alineado con `fns[i]`, G27; `race` cancela a las perdedoras —ojo: la cancelación real es S08, así que S07 documentará cómo aborta sin ella).
